...
stokesV=np.empty_like(file0, dtype=np.int32)
chunksize=256347
for i in range(32):
    stokesV[i*chunksize:(i+1)*chunksize,:] =2*(
    (file0 [i * chunksize: (i+1) * chunksize, :].astype(np.int32))*(file3 [i * chunksize: (i+1) * chunksize, :].astype(np.int32)) -
    (file2 [i * chunksize: (i+1) * chunksize, :].astype(np.int32))*(file1 [i * chunksize: (i+1) * chunksize, :].astype(np.int32)))


np.save("1936_V.npy", stokesV)
"""
im=plt.imshow(stokesV, cmap="viridis", aspect="auto", vmin=np.min(stokesV), vmax=np.max(stokesV))
cbar=plt.colorbar(im)
cbar.set_label("Stokes V")
plt.title("Stokes V")
plt.xlabel("Freq Channel")
plt.ylabel("Time")
#plt.savefig("V Stokes")
