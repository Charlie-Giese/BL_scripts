...
for i in range(Nsets):
    ...
    with open(path+"/crab_giant_pulses3_2020-03-17T19:25:13,_17901936.rawudp", "rb") as f:
        ...
        del map3
    f.close()
    stokesU=np.empty_like(x_real, dtype=np.int32) 
    chunks=13
    chunksize=int(x_real.shape[0]/chunks)
    for j in range(chunks):
        stokesU[j*chunksize:(j+1)*chunksize] =2*(
    ...
    output=open('/mnt/ucc2_data1/data/giesec/crab/raw_filterbank_files/1936_U.fil', 'ab')
    output.write(stokesU)
    output.close()
