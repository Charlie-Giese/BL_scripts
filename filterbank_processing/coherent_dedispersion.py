...
x_imag=file0.reshape(-1, 488)
y_real=file0.reshape(-1, 488)
y_imag=file0.reshape(-1, 488)
...
def freqs(nbands, f_top, f_off): #in MHz
    #print(nbands, f_top, f_off)
    f_min=f_top+(nbands*f_off)
    #print(f_min)
    freqs=np.arange(f_top, f_min, f_off)
    return freqs

def stokesI(f_top, f_off, sample_rate, dm):
    ...
    ydd=np.zeros_like(y)
    nbands=x.shape[1]
    f0=freqs(nbands, f_top, f_off)
    t=times(x, sample_rate)
    #print(np.shape(f0))
    for i in xrange(nbands):
        temp= coherent(t, x[:, i], f0[i], sample_rate, dm, taper=False, 
        previousTime=None, previousData=None, nextTime=None, nextData=None, enableCaching=True)
        xdd[:, i]=temp[0]

    for i in xrange(nbands):
        temp= coherent(t, y[:, i], f0[i], sample_rate, dm, taper=False,
        previousTime=None, previousData=None, nextTime=None, nextData=None, enableCaching=True)
        ydd[:, i]=temp[0]
    ...
    for i in xrange(256):
        ...
        np.square(x_coherently_dd[i * chunksize: (i+1) * chunksize, :].astype(np.int32)) +
        np.square(y_coherently_dd[i * chunksize: (i+1) * chunksize, :].astype(np.int32)))
    return stokesI


print(np.shape(stokesI(197.55859375, -0.1953125, 5.12*(10**-6), 56.77)))
